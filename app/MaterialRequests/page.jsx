...
export default function MaterialRequestsView() {
    ...
    const fetchList = async () => {
        ...
        try {
            ...
            const res = await fetch(`/api/material-requests/get?${params.toString()}`);
            if (res.ok) {
    ...
    const handleExportClick = async () => {
        ...
        try {
            await exportToExcel(processedData, "MaterialRequests"); 
    ...
    const columns = [
        ...
        { key: 'Req_Dt', label: 'Date', w: 'w-[120px]' },
        { key: 'Item_Code', label: 'Item Code', w: 'w-[150px]' },
    ...
    return (
        <div className="@container/main flex flex-col h-screen overflow-hidden bg-background space-y-2 font-sans w-full">
            <div className="flex-none flex flex-col md:flex-row items-start md:items-center justify-between bg-card p-4 rounded-lg shadow-sm border border-border gap-4">
                <div>
                    <h1 className="text-xl font-bold text-primary">Material Requests</h1>
                </div>
