...
import { Filter, X, Search, ChevronsUpDown, Trash2, MoreHorizontal, Edit, ArrowLeft, RotateCcw, Save, Loader, Check, ArrowUp, ArrowDown, Calendar as CalendarIcon } from 'lucide-react';
...
const EditForm = ({ item, onSave, onCancel }) => {
    ...
    useEffect(() => {
        const fetchShifts = async () => {
            try {
                const response = await fetch('/api/options/shifts');
                const data = await response.json();
                setShiftOptions(data);
            } catch (error) {
                console.error("Failed to fetch shifts for edit form", error);
            }
        };
        fetchShifts();
    }, []);
    ...
    const handleReset = () => setFormData({...item, TransactionDate: new Date(item.TransactionDate)});
    ...
    const handleSubmit = async (e) => {
        ...
        try { 
            ...
            const payload = { ...formData, TransactionDate: format(formData.TransactionDate, 'yyyy-MM-dd'), ModifiedBy: employeeId };
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <div className="flex items-center justify-between mb-6 px-2"><h2 className="text-2xl font-bold">Edit Record (ID: {item.Id})</h2><Button variant="ghost" onClick={onCancel}><ArrowLeft className="mr-2 h-4 w-4" /> Back to Table</Button></div>
            <form onSubmit={handleSubmit} className="bg-card border rounded-lg p-6 space-y-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="space-y-2"><Label>Size Specifications</Label><Input value={formData.SizeSpecifications || ''} readOnly disabled className="cursor-not-allowed" /></div>
                    <div className="space-y-2"><Label>Date</Label><Popover><PopoverTrigger asChild><Button variant={"outline"} className={cn("w-full justify-start text-left font-normal")}><CalendarIcon className="mr-2 h-4 w-4" />{formData.TransactionDate ? format(formData.TransactionDate, "PPP") : <span>Pick a date</span>}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={formData.TransactionDate} onSelect={(date) => setFormData(p=>({...p, TransactionDate:date}))} /></PopoverContent></Popover></div>
                    <div className="space-y-2">
                        <Label>Shift</Label>
                        <Popover><PopoverTrigger asChild><Button variant="outline" role="combobox" className={cn("w-full justify-between", !formData.Shift && "text-muted-foreground")}>{formData.Shift || "Select shift..."}<ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" /></Button></PopoverTrigger><PopoverContent className="w-[--radix-popover-trigger-width] p-0"><Command><CommandInput placeholder="Search shift..." /><CommandEmpty>No shift found.</CommandEmpty><CommandGroup>{shiftOptions.map((option) => (<CommandItem key={option.value} value={option.value} onSelect={() => handleInputChange({ target: { name: 'Shift', value: option.value } })}><Check className={cn("mr-2 h-4 w-4", formData.Shift === option.value ? "opacity-100" : "opacity-0")} />{option.label}</CommandItem>))}</CommandGroup></Command></PopoverContent></Popover>
                    </div>
                    <div className="space-y-2"><Label>Full Order Qty</Label><Input name="FullOrderQuantity" type="number" value={formData.FullOrderQuantity || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>OK Qty</Label><Input name="OKQuantity" type="number" value={formData.OKQuantity || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>NG Qty</Label><Input name="NGQuantity" type="number" value={formData.NGQuantity || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Total Time</Label><Input name="TotalOrderCompletionTime" type="number" step="0.01" value={formData.TotalOrderCompletionTime || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Cycle Time/Pc</Label><Input name="CycleTimePerPiece" type="number" step="0.01" value={formData.CycleTimePerPiece || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Debug Time</Label><Input name="ToolDebuggingTime" type="number" value={formData.ToolDebuggingTime || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Rework Qty</Label><Input name="ReworkQty" type="number" value={formData.ReworkQty || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Rework Time</Label><Input name="ReworkTime" type="number" step="0.01" value={formData.ReworkTime || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Warm Up</Label><Input name="WarmUp" type="number" value={formData.WarmUp || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Idle Time</Label><Input name="IdleTime" type="number" value={formData.IdleTime || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Breakdown Time</Label><Input name="EquipmentBreakdownTime" type="number" value={formData.EquipmentBreakdownTime || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2"><Label>Tools NA Time</Label><Input name="ToolsNotAvailableTime" type="number" value={formData.ToolsNotAvailableTime || ''} onChange={handleInputChange} /></div>
                    <div className="space-y-2 sm:col-span-2 lg:col-span-4"><Label>Remarks</Label><Textarea name="Remarks" value={formData.Remarks || ''} onChange={handleInputChange} /></div>
                </div>
                <div className="flex justify-end items-center gap-4 pt-8">
                    <Button type="button" variant="outline" onClick={handleReset}><RotateCcw className="mr-2 h-4 w-4" /> Reset</Button>
                    <Button type="submit" className={cn("w-36", { "bg-green-500": submitStatus === 'success', "bg-red-500": submitStatus === 'error' })} disabled={submitStatus !== 'idle'}>
                        <AnimatePresence mode="wait"><motion.div key={submitStatus} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -10, opacity: 0 }} className="flex items-center justify-center">
                            ...
                            {submitStatus === 'idle' && <><Save className="mr-2 h-4 w-4" /> Save Changes</>}
...
export default function ViewWalterBasicClient() {
    ...
    const [headerSearchColumn, setHeaderSearchColumn] = useState('ItemCode');
    const [headerSearchValues, setHeaderSearchValues] = useState([]);
    const [dateRange, setDateRange] = useState({ from: subDays(new Date(), 30), to: new Date() });

    const dateColumns = ['TransactionDate', 'CreatedAt', 'ModifiedAt'];

    const fetchData = useCallback(async (currentDateRange) => {
        if (!currentDateRange?.from || !currentDateRange?.to) { setBasicData([]); setIsLoading(false); return; }
        setIsLoading(true);
        try {
            const response = await fetch('/api/walter/basic/get', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ dateRange: currentDateRange }) });
    ...
    const uniqueColumnValues = useMemo(() => {
        ...
        const values = new Set(basicData.map(item => String(item[headerSearchColumn] || '').trim()));
        return Array.from(values).filter(Boolean).map(v => ({ label: v, value: v }));
    }, [headerSearchColumn, basicData]);

    const handleEdit = (item) => { setEditingItem(item); setViewMode('edit'); };

    const handleUpdate = async (updatedItem) => {
        try {
            const response = await fetch('/api/walter/basic/update', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(updatedItem) });
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`/api/walter/basic/delete?id=${id}`, { method: 'DELETE' });
            if (!response.ok) { const err = await response.json(); throw new Error(err.message); }
    ...
    const handleHeaderColumnChange = (value) => { setHeaderSearchColumn(value); setHeaderSearchValues([]); };
    ...
    const processedData = useMemo(() => {
        ...
        if (globalFilter) data = data.filter(item => Object.values(item).some(val => String(val).toLowerCase().includes(globalFilter.toLowerCase())));
        ...
        if (activeColFilters.length > 0) data = data.filter(item => activeColFilters.every(([key, values]) => {
            const itemValue = item[key];
            if (dateColumns.includes(key)) {
                const formatted = itemValue ? format(new Date(itemValue), "dd MMM, yyyy, h:mm a") : null;
                return formatted ? values.includes(formatted) : values.includes('');
            }
            return values.includes(String(itemValue));
        }));
        if (sortConfig) data.sort((a, b) => {
        ...
        return data;
    }, [headerSearchValues, headerSearchColumn, globalFilter, appliedFilters, columnFilters, sortConfig, basicData]);

    const totalPages = Math.ceil(processedData.length / pagination.rowsPerPage);
    const paginatedData = useMemo(() => processedData.slice((pagination.currentPage - 1) * pagination.rowsPerPage, pagination.currentPage * pagination.rowsPerPage), [processedData, pagination]);

    useEffect(() => { if (pagination.currentPage > totalPages && totalPages > 0) setPagination(p => ({ ...p, currentPage: totalPages })); }, [pagination.currentPage, totalPages]);
    useLayoutEffect(() => {
        const calculateHeight = () => {
            if (containerRef.current) {
                const header = containerRef.current.querySelector('#page-header');
                const footer = containerRef.current.querySelector('#pagination-container');
                const offset = containerRef.current.getBoundingClientRect().top;
                const reserved = (header?.offsetHeight || 0) + (footer?.offsetHeight || 0);
                setTableHeight(`${Math.max(400, window.innerHeight - offset - reserved - 32)}px`);
            }
        };
        calculateHeight(); window.addEventListener('resize', calculateHeight); return () => window.removeEventListener('resize', calculateHeight);
    }, [processedData.length]);

    const handleExport = async () => {
        if (processedData.length === 0) { toast.error("No Data"); throw new Error("No data"); }
        ...
        await exportToExcel({ arrayOfArrays: [headers, ...dataRows], filename: `WalterBasicData_${format(new Date(), 'yyyyMMdd_HHmm')}` });
    };

    if (viewMode === 'edit') return <EditForm item={editingItem} onSave={handleUpdate} onCancel={() => setViewMode('table')} />;
        ...
        <div ref={containerRef} className="@container/main bg-card border rounded-xl shadow-lg flex flex-col h-full overflow-hidden">
            <div id="page-header" className="p-4 space-y-4">
                <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                    <div><h1 className="text-2xl font-bold text-brand-500">Walter Basic Data</h1></div>
                ...
                <div className="space-y-2">
                    ...
                    <div className="relative"><Search className="h-5 w-5 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" /><Input type="text" placeholder="Global search..." value={globalFilter} onChange={(e) => setGlobalFilter(e.target.value)} className="w-full pl-10" /></div>
                </div>
            </div>
                ...
                error ? (<div className="text-center p-16 text-destructive">Error: {error}</div>) :
                    (<div className="overflow-auto" style={{ height: tableHeight }}>
                        <table className="w-full text-sm">
                            <thead className="bg-muted sticky top-0 z-10"><tr>
                                {allColumns.map((col) => (<th key={col.key} className="px-4 py-2 text-left font-medium text-muted-foreground whitespace-nowrap text-xs">
                                    <div className="flex items-center gap-1.5 select-none">
                                        <span onClick={() => handleSort(col.key)} className="cursor-pointer group flex items-center gap-1.5">{col.label}
                                            <span className="opacity-50 group-hover:opacity-100">{sortConfig?.key === col.key ? (sortConfig.direction === 'descending' ? <ArrowDown className="h-4 w-4" /> : <ArrowUp className="h-4 w-4" />) : <ChevronsUpDown className="h-4 w-4" />}</span>
                                        </span>
                                        {col.filterable && (<ColumnFilterPopover column={col} data={basicData} activeFilters={columnFilters} onFilterChange={handleColumnFilterChange} dateColumns={dateColumns} />)}
                                    </div>
                                </th>))}
                            ...
                            <tbody>{paginatedData.map((item) => (
                                <tr key={item.Id} className="border-b hover:bg-muted/50 bg-card">
                                    {allColumns.map((col) => (<td key={col.key} className="px-4 py-1 align-middle whitespace-nowrap text-xs">
                                        ...
                                        <div className="max-w-[200px] truncate" title={String(item[col.key] ?? '')}>{String(item[col.key] ?? '')}</div>}
                                    </td>))}
                                    <td className="sticky right-0 bg-card w-[100px] text-center px-4 py-1">
                                        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" className="h-8 w-8 p-0"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end">
                                            ...
                                            <DropdownMenuItem onClick={() => setItemToDelete(item.Id)} className="text-destructive focus:text-destructive"><Trash2 className="mr-2 h-4 w-4" /> Delete</DropdownMenuItem>
                                        </DropdownMenuContent></DropdownMenu>
                                    </td>
                                </tr>))}
                            </tbody>
                        </table>
                        {processedData.length === 0 && !isLoading && (<div className="text-center p-16"><p className="font-bold text-lg">No records found.</p><p className="text-muted-foreground">Try adjusting your filter criteria.</p></div>)}
                    </div>)}
            {processedData.length > 0 && !isLoading && <div id="pagination-container" className="p-4 border-t flex flex-col md:flex-row items-center justify-between gap-4"><div className="flex items-center gap-2"><span>Rows:</span><select value={pagination.rowsPerPage} onChange={(e) => setPagination({ currentPage: 1, rowsPerPage: Number(e.target.value) })} className="bg-background border rounded-md p-1">{[10, 20, 50, 100].map(size => <option key={size} value={size}>{size}</option>)}</select></div><span>{((pagination.currentPage - 1) * pagination.rowsPerPage) + 1}-{Math.min(pagination.currentPage * pagination.rowsPerPage, processedData.length)} of {processedData.length}</span><div className="flex items-center gap-2"><Button variant="outline" size="icon" onClick={() => setPagination(p => ({ ...p, currentPage: 1 }))} disabled={pagination.currentPage === 1}>«</Button><Button variant="outline" size="icon" onClick={() => setPagination(p => ({ ...p, currentPage: p.currentPage - 1 }))} disabled={pagination.currentPage === 1}>‹</Button><Button variant="outline" size="icon" onClick={() => setPagination(p => ({ ...p, currentPage: p.currentPage + 1 }))} disabled={pagination.currentPage === totalPages}>›</Button><Button variant="outline" size="icon" onClick={() => setPagination(p => ({ ...p, currentPage: totalPages }))} disabled={pagination.currentPage === totalPages}>»</Button></div></div>}
