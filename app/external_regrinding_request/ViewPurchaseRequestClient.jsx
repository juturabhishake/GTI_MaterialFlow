...
export default function PurchaseRequestClient() {
    const PAGE_ID_FOR_THIS_FORM = 3037;
    ...
    const fetchDataForDate = useCallback(async (d) => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch('/api/ExternalRegrindingRequest/get', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ reqDate: format(d, 'yyyy-MM-dd') }) });
    ...
    const handleAddItem = async (itemCode) => {
        ...
        try {
            const response = await fetch('/api/ExternalRegrindingRequest/item-details', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ itemCode }) });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/ExternalRegrindingRequest/upsert', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ data: payload }) });
    ...
    const handleRemoveRow = async (row) => {
        if (row.Id > 0) { try { await fetch(`/api/ExternalRegrindingRequest/delete?id=${row.Id}`, { method: 'DELETE' }); fetchDataForDate(date); } catch { toast.error("Delete failed"); } } else { setTableRows(prev => prev.filter(r => r.tempId !== row.tempId)); }
    };
        ...
        <div ref={containerRef} className="@container/main bg-card border rounded-xl shadow-lg flex flex-col h-full overflow-hidden">
            <div id="header-section" className="p-4 space-y-4">
                <header className="flex flex-wrap lg:flex-row items-start lg:items-center justify-between gap-4">
                    <h1 className="text-xl sm:text-2xl font-bold text-brand-500 whitespace-nowrap">Ext. Regrinding Request</h1>
