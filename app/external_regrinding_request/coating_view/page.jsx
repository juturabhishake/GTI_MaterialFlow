...
import { Loader2, FileSpreadsheet, ListFilter, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ArrowUpDown, ArrowUp, ArrowDown, Check, FileText, X, Undo2 } from "lucide-react";
...
export default function ToolmakingReturnView() {
    const PAGE_ID_FOR_THIS_FORM = 3040;
    ...
    const fetchList = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/ext_coating/get-returns`);
            if (res.ok) {
    ...
    const handleSort = (key) => {
        if (['checkbox', 'process', 'is_Return_to_Toolmaking', 'good_parts_by_coating', 'comments_by_coating'].includes(key)) return;
        let direction = 'asc';
    ...
    const getSortIcon = (key) => {
        if (['checkbox', 'process', 'is_Return_to_Toolmaking', 'good_parts_by_coating', 'comments_by_coating'].includes(key)) return null;
    ...
    const handleExportClick = async () => {
        ...
        try {
            await exportToExcel(processedData, "ReturnToToolmaking"); 
    ...
    const handleRowChange = (id, field, value) => {
        setData(prev => prev.map(row => {
            if (row.Id === id) {
                ...
                if (field === 'good_parts_by_coating') {
                    const goodVal = value === '' ? null : parseFloat(value);
                    if (goodVal === null) {
                        updatedRow.good_parts_by_coating = null;
                        updatedRow.rejected_parts_by_coating = null;
                    } else {
                        const completed = parseFloat(updatedRow.Completed_parts_qty) || 0;
                        const rejected = completed - goodVal;
                        updatedRow.good_parts_by_coating = goodVal;
                        updatedRow.rejected_parts_by_coating = rejected >= 0 ? rejected : 0;
                    }
                }
                return updatedRow;
            }
    ...
    const handleReturnToToolmaking = async (rowsToReturn = null) => {
        const targetRows = rowsToReturn || data.filter(r => r.isSelected && !r.is_Return_to_Toolmaking);
        
        if (targetRows.length === 0) {
            toast.warning("No pending records selected to return.");
            return;
        }
        ...
        try {
            ...
            targetRows.forEach(row => {
                ...
                xmlPayload += `<Row>
                    ...
                    <Return_to_Toolmaking_by_name>${currentUser.name}</Return_to_Toolmaking_by_name>
                    <Return_to_Toolmaking_by_empid>${currentUser.empId}</Return_to_Toolmaking_by_empid>
                    <Return_to_Toolmaking_by_at>${format(new Date(), "yyyy-MM-dd")}</Return_to_Toolmaking_by_at>
                </Row>`;
            });
            xmlPayload += '</Root>';

            const response = await fetch("/api/ext_coating/return-to-toolmaking", {
                method: "POST",
            ...
            toast.success(`Successfully returned ${targetRows.length} items to toolmaking.`);
            fetchList();
        } catch (e) {
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            if (isCmdOrCtrl && !e.shiftKey && (e.key === 'a' || e.key === 'A')) {
                ...
                const areAllFilteredSelected = eligibleFilteredRows.every(r => r.isSelected);
                const eligibleFilteredIds = new Set(eligibleFilteredRows.map(r => r.Id));

                setData(prev => prev.map(row => {
                    if (eligibleFilteredIds.has(row.Id)) {
                        return { ...row, isSelected: !areAllFilteredSelected };
                    }
                    return row;
                }));

                if (areAllFilteredSelected) toast.info("Filtered selections cleared.");
    ...
    const columns = [
        ...
        // { key: 'is_Return_to_Toolmaking', label: 'Return to Toolmaking', w: 'w-[200px]' },
        ...
        { key: 'Completed_parts_qty', label: 'Order Qty', w: 'w-[120px]' },
        { key: 'good_parts_by_coating', label: 'Good Parts', w: 'w-[120px]' },
        { key: 'rejected_parts_by_coating', label: 'Rejected Parts', w: 'w-[120px]' },
    ...
    return (
        <div className="@container/main flex flex-col h-screen overflow-hidden bg-background space-y-2 font-sans w-full">
            <div className="flex-none flex flex-col md:flex-row items-start md:items-center justify-between bg-card p-4 rounded-lg shadow-sm border border-border gap-4">
                <div>
                    <h1 className="text-xl font-bold text-primary">Return to Toolmaking</h1>
                </div>
                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
                    <Button 
                        onClick={() => handleReturnToToolmaking()}
                        disabled={!hasSelectedItems || isSaving}
                        className={cn("transition-all duration-300 gap-2 cursor-pointer bg-purple-600 hover:bg-purple-700", isSaving && "opacity-80")}
                    >
                        {isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Undo2 className="h-4 w-4" />}
                        Return Selected
                    </Button>
                    <Button 
                                                ...
                                                <td key={col.key} className={cn("p-2 border-r border-border", col.key === 'ItemSpecification' && "max-w-[200px] truncate")} title={row[col.key]}>
                                                    {col.key === 'RequestId' ? `GTI-${new Date(row.ReqDate).getFullYear()}-RG-${row.RequestId}` : 
                                                     col.key === 'checkbox' ? (
                                                        <input 
                                                            type="checkbox" 
                                                            checked={!!row.is_Return_to_Toolmaking || !!row.isSelected}
                                                            disabled={!!row.is_Return_to_Toolmaking} 
                                                            onChange={() => toggleRowSelection(row.Id)} 
                                                            className="w-4 h-4 cursor-pointer" 
                                                        />
                                                    ) : col.key === 'process' ? (
                                                        !row.is_Return_to_Toolmaking ? (
                                                            <Button size="sm" variant="outline" className="h-6 text-[10px] w-full gap-1 cursor-pointer border-purple-600 text-purple-600 hover:bg-purple-50" onClick={() => handleReturnToToolmaking([row])} disabled={isSaving}>
                                                                <Undo2 className="h-3 w-3" /> Return
                                                            </Button>
                                                        ) : <div className="flex items-center gap-1 font-semibold text-purple-600">
                                                                <Check className="h-3 w-3 shrink-0" />
                                                                <span className="truncate">{row.Return_to_Toolmaking_by_name}({row.Return_to_Toolmaking_by_empid}) - {row.Return_to_Toolmaking_by_at ? format(new Date(row.Return_to_Toolmaking_by_at), 'dd/MM/yyyy') : ''}</span>
                                                            </div>
                                                    ) : col.key === 'is_Return_to_Toolmaking' ? (
                                                        row.is_Return_to_Toolmaking ? (
                                                            <div className="flex items-center gap-1 font-semibold text-purple-600">
                                                                <Check className="h-3 w-3 shrink-0" />
                                                                <span className="truncate">{row.Return_to_Toolmaking_by_name}({row.Return_to_Toolmaking_by_empid}) - {row.Return_to_Toolmaking_by_at ? format(new Date(row.Return_to_Toolmaking_by_at), 'dd/MM/yyyy') : ''}</span>
                                                    ...
                                                    ) : col.key === 'good_parts_by_coating' ? (
                                                        <Input
                                                            className="h-6 text-[10px] w-full border-primary/50"
                                                            type="number"
                                                            disabled={!!row.is_Return_to_Toolmaking}
                                                            value={row.good_parts_by_coating ?? ""}
                                                            onChange={(e) => handleRowChange(row.Id, 'good_parts_by_coating', e.target.value)}
                                                        />
                                                    ) : col.key === 'rejected_parts_by_coating' ? (
                                                        <span className="font-medium text-muted-foreground bg-muted/30 rounded px-2 py-1 border">{row.rejected_parts_by_coating ?? "-"}</span>
                                                    ) : col.key === 'comments_by_coating' ? (
                                                        <Input
                                                            className="h-6 text-[10px] w-full border-primary/50"
                                                            disabled={!!row.is_Return_to_Toolmaking}
                                                            value={row.comments_by_coating || ""}
                                                            onChange={(e) => handleRowChange(row.Id, 'comments_by_coating', e.target.value)}
