...
export default function CoatingSendView() {
  const PAGE_ID_FOR_THIS_FORM = 3039;
    ...
    const fetchList = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/ExternalRegrindingRequest/get-all`);
            if (res.ok) {
    ...
    const handleSendToCoating = async (rowsToSend = null) => {
        ...
        try {
            ...
            const response = await fetch("/api/ExternalRegrindingRequest/send-to-coating", {
                method: "POST",
    ...
    const eligibleFilteredRowsForHeader = processedData.filter(r => !r.Send_to_coating);
