...
export default function PurchaseRequestDetail({ request, onBack }) {
  ...
  const fetchRequestData = async (reqId) => {
    setIsLoading(true);
    try {
      const res = await fetch("/api/ExternalRegrindingRequest/get-by-id", {
        method: "POST",
  ...
  const handleAddItem = async (itemCode) => {
    ...
    try {
      const response = await fetch("/api/ExternalRegrindingRequest/item-details", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSave = async (approvalData = null) => {
    ...
    try {
      ...
      const response = await fetch("/api/ExternalRegrindingRequest/upsert", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (request) {
        ...
        const fetchRes = await fetch("/api/ExternalRegrindingRequest/get-by-id", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ requestId: request.RequestId }),
        });
  ...
  const handleRemoveRow = async (row) => {
    if (row.Id > 0) {
      try {
        await fetch(`/api/ExternalRegrindingRequest/delete?id=${row.Id}`, {
          method: "DELETE",
        });
      ...
      return (
        <Button
          ...
          onClick={async () => {
            ...
            try {
              const res = await fetch("/api/ExternalRegrindingRequest/complete-row", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
