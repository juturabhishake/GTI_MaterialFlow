  ...
  return (
    <div className="flex items-center w-full gap-2 pointer-events-none">
      <div className="flex items-center scale-75 origin-left">
        ...
        <div className="h-[2px] w-3 bg-gray-200 mx-0.5">
          <div
            className={cn("h-full bg-green-500", hosApp ? "w-full" : "w-0")}
          ></div>
        </div>
        ...
        <div className="h-[2px] w-3 bg-gray-200 mx-0.5">
          <div
            className={cn("h-full bg-green-500", recApp ? "w-full" : "w-0")}
          ></div>
        </div>
...
export default function PurchaseRequestSummary({ onViewDetails, onCreateNew }) {
  ...
  const fetchSummary = useCallback(async (start, end) => {
    ...
    try {
      ...
      const formatted = result.map((row) => {
        ...
        let statusVal = "Pending HOS";
        if (hos && !rec) statusVal = "Pending REC";
        if (hos && rec && !fin) statusVal = "Pending Final";
        if (hos && rec && fin) statusVal = "Approved";
  ...
  const renderCell = (row, col) => {
    if (col.key === "Status") {
      ...
      const hosApp = !!row.isHOSApproved;
      const recApp = !!row.isReceiverApproved;
      const finApp = !!row.is_Final_HOS_Approval;
      ...
      const Step = ({ active, done, label, title }) => (
        <div className="flex flex-col items-center w-8" title={title}>
          ...
          <span className="text-[8px] mt-1 font-bold text-muted-foreground uppercase">
            {label}
          </span>
        </div>
      );
      ...
      return (
        <div className="flex items-center py-1">
          <Step
            ...
            active={!hosApp}
            label="HOS"
            title={hosApp ? `By: ${row.HOSName || "HOS"}` : "Pending HOS"}
          />
          <Line done={hosApp} />
          <Step
            done={recApp}
            active={hosApp && !recApp}
            label="REC"
            title={
              recApp
                ? `By: ${row.ReceiverName || "Receiver"}`
                : "Pending Receiver"
            }
          />
          <Line done={recApp} />
          <Step
            ...
            active={recApp && !finApp}
            label="FIN"
            title={
              finApp
                ? `By: ${row.Final_HOS_Name || "Final HOS"}`
