...
const allColumns = [
    ...
    { key: 'CheckNGQty', label: 'Check NG', filterable: true },
    { key: 'DetermineOrderQty', label: 'Det. Order Qty', filterable: true },
    { key: 'ConfirmationByUser', label: 'Confirmed By User Section', filterable: true },
...
export default function PurchaseRequestClient() {
    ...
    const handleAddItem = async (itemCode) => {
        ...
        try {
            ...
            const newRow = { Id: 0, tempId: `new-${Date.now()}-${Math.random()}`, ReqDate: date, MaterialCode: itemCode, ItemSpecification: details.ItemSpecification || details.SizeSpecifications || '', ProjectName: details.ProjectName || '', OrderQty: null, CheckNGQty: null, DetermineOrderQty: null, DemandDate: null, ConfirmationByUser: null, Remarks: null };
    ...
    return (
        <div ref={containerRef} className="@container/main bg-card border rounded-xl shadow-lg flex flex-col h-full overflow-hidden">
            ...
            {isLoading ? (<div className="flex justify-center items-center flex-grow"><Loader2 className="h-10 w-10 animate-spin text-primary" /></div>) : (
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-muted sticky top-0 z-10">
                            <tr>
                                ...
                                {allColumns.map(col => (<th key={col.key} className="px-4 py-2 text-left font-medium whitespace-nowrap"><div className="flex items-center gap-1">{col.label}{col.filterable && <ColumnFilterPopover column={col} data={tableRows} activeFilters={columnFilters} onFilterChange={handleColumnFilterChange} />}</div></th>))}
                        ...
                        <tbody>
                            {paginatedData.map((row, i) => (
                                <tr key={row.tempId} className="border-b hover:bg-muted/50">
                                    ...
                                    <td className="px-4 py-1"><Input type="number" className="h-7 w-20 text-xs min-w-[80px]" value={row.OrderQty || ''} onChange={e => handleRowChange(row.tempId, 'OrderQty', e.target.value)} /></td>
                                    ...
                                    <td className="px-4 py-1"><Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className="h-7 w-[100px] text-[10px] justify-start px-2">{row.DemandDate ? format(row.DemandDate, "dd-MMM-yy") : "Pick"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={row.DemandDate} onSelect={d => handleRowChange(row.tempId, 'DemandDate', d)} /></PopoverContent></Popover></td>
                                    <td className="px-4 py-1"><Input className="h-7 w-32 text-xs min-w-[120px]" value={row.Remarks || ''} onChange={e => handleRowChange(row.tempId, 'Remarks', e.target.value)} /></td>
                                    <td className="px-4 py-1 text-center"><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => handleRemoveRow(row)}><Trash2 className="h-3.5 w-3.5" /></Button></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
